...
/* Problem Statement: Remove Redundant Pairs - SQL Interview Query 1 | SQL Problem Level "HARD"
...
with cte as (
select *, 
case when brand1 < brand2 then concat(brand1, brand2)
	 else concat(brand2,brand1)  end as pair
from brands),

cte2 as (
select *, row_number() over(partition by year,pair order by year) as rn from cte )
